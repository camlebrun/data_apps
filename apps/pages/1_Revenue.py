...
revenues_per_month.index = revenues_per_month.index.astype(
    str)  # Convert Period index to string
fig_revenue_per_month = px.bar(
    revenues_per_month,
    x=revenues_per_month.index,
    y='payment_value',
    title='Revenues per Month',
    labels={
        'payment_value': 'Payment Value'})

# Add text annotations for each data point
for i, value in enumerate(revenues_per_month):
    fig_revenue_per_month.add_annotation(
        x=revenues_per_month.index[i],
        y=value,
        text=f"${value:.2f}",
        showarrow=True,
        arrowhead=2)
...
revenues_per_year.index = revenues_per_year.index.astype(
    str)  # Convert Period index to string
# Create a line plot using Plotly Express
fig_payment_value_year = px.bar(
    revenues_per_year,
    x=revenues_per_year.index,
    y='payment_value',
...
with col2:
    ...
    fig = px.pie(
        revenues_per_payment_type,
        values='payment_value',
        names=revenues_per_payment_type.index,
...
sales_per_state['payment_value'] = pd.to_numeric(
    sales_per_state['payment_value'])

# Calculate total sales
total_sales = sales_per_state['payment_value'].sum()

# Calculate percentage
sales_per_state['percentage'] = (
    sales_per_state['payment_value'] / total_sales) * 100

# Format the text to be displayed on the bars
sales_per_state['text'] = sales_per_state.apply(
    lambda row: f"${row['payment_value']:.2f} ({row['percentage']:.2f}%)", axis=1)

# Plot the bar chart with data labels and percentages
fig_sales_per_state = px.bar(
    sales_per_state,
    x='customer_state',
    y='payment_value',
    text='text',
...
fig_sales_per_state.update_traces(
    texttemplate='%{text}',
    textposition='outside')
fig_sales_per_state.update_layout(
...
fig_customers_per_state = px.bar(
    customers_per_state,
...
with col1:
    st.markdown(
        '<h3 style="text-align: center;">Revenues per State</h3>',
        unsafe_allow_html=True)
...
selected_region_data['Year'] = selected_region_data['order_purchase_timestamp'].dt.year.astype(
    str)
...
fig_revenue_per_state_per_year = px.bar(
    selected_data, x='Year', y='payment_value')
