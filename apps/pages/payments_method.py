...
st.warning("Please note that only percentages greater than or equal to 1% are displayed.")
        ...
        for payment_type, count in value.items():
            ...
            if percentage >= 1:  # Check if percentage is greater than or equal to 1
                ...
                if payment_type in ['credit_card', 'boleto']:
                    col1.metric(formatted_payment_type, value=count, delta=delta_percentage)
                else:
                    col2.metric(formatted_payment_type, value=count, delta=delta_percentage)
...
fig = px.pie(selected_region_data, values=selected_region_data.values, names=selected_region_data.index, title=f'Payment Methods for {selected_region}', labels={'value': 'Percentage'})
...
sales_per_state = sales_per_state.sort_values(by='payment_value', ascending=True)  # Sort in ascending order
