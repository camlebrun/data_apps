...
class KpiCalculator:
    ...
    def payments_method_per_region(self):
        paym_type = self.data.groupby(['customer_state','payment_type'])['order_id'].count().sort_values(ascending=False)
        return {'Payment type usage': paym_type}
    ...
    def revenues_per_payment_type_per_year(self):
        ...
        filtered_data = self.data[self.data['order_status'] != 'canceled']
        filtered_data['order_purchase_timestamp'] = pd.to_datetime(filtered_data['order_purchase_timestamp'])
        return filtered_data.groupby([filtered_data['order_purchase_timestamp'].dt.to_period("Y"), 'payment_type'])['payment_value'].sum()
    def revenues_per_state_per_year(self, selected_region):
        ...
        filtered_data = filtered_data[filtered_data['customer_state'] == selected_region]
        
        # Group by year and sum payment values, reset index to convert the result into a DataFrame
        result = filtered_data.groupby(filtered_data['order_purchase_timestamp'].dt.to_period("Y"))['payment_value'].sum().reset_index()
    ...
    def count_orders_over_time_days(self):
        ...
        filtered_data['order_purchase_timestamp'] = pd.to_datetime(filtered_data['order_purchase_timestamp'])
        # Group by date and count the number of orders, reset index to convert the result into a DataFrame
        result = filtered_data.groupby(filtered_data['order_purchase_timestamp'].dt.to_period("D"))['order_id'].count().reset_index()
    ...
    def orders_status(self):
        ...
        orders_status = filtered_data['order_status'].value_counts(normalize=True).mul(100).round(1)
        
        # Create a DataFrame with the order status and their respective proportions
        orders_status_df = pd.DataFrame({'order_status': orders_status.index, 'proportion': orders_status.values})
    ...
    def revenues_per_custom_period(self, period='year', start=None, end=None):
        ...
        if period == 'year':
            revenues = filtered_data.groupby(filtered_data['order_purchase_timestamp'].dt.to_period("Y"))['payment_value'].sum()
        else:
            raise ValueError("Invalid period. Please use 'year'.")

        return revenues
    def revenues_and_orders_per_custom_period(self, period='year', start=None, end=None):
        # Filter out canceled orders and ensure 'order_purchase_timestamp' is in datetime format
        filtered_data = self.data[self.data['order_status'] != 'canceled']
        filtered_data['order_purchase_timestamp'] = pd.to_datetime(filtered_data['order_purchase_timestamp'])
        ...
        if period == 'year':
            revenues = filtered_data.groupby(filtered_data['order_purchase_timestamp'].dt.to_period("Y"))['payment_value'].sum()
            orders = filtered_data.groupby(filtered_data['order_purchase_timestamp'].dt.to_period("Y"))['order_id'].count()
        else:
            raise ValueError("Invalid period. Please use 'year'.")
